...
node_stats_better = df_with_nodes %>%
  ...
  mutate(
    # Weight by BOTH size AND quality (better rate)
    sum_better = sum(mean_better), # add this otherwise you are ignoring the fact that ANY worse terminal node has small amount of worse in it
    weight = n_orig * mean_better,
    proportion = weight / sum(weight),
    n_to_sample = round(n_per_better*proportion)
    # n_to_sample = round((1/sum_better*n_per_better)*proportion)
  )

print("BETTER tree nodes with quality weights:")
print(node_stats_better %>% 
        select(node_better, n_orig, mean_better, weight, n_to_sample) %>%
        arrange(desc(mean_better)))

# Calculate the RATE of worse outcomes in each node of the worse tree
node_stats_worse = df_with_nodes %>%
  group_by(node_worse) %>%
  summarise(
    n_orig = n(),
    mean_better = mean(frac_better),
    mean_worse = mean(frac_worse),      # THIS IS THE KEY!
    mean_drift = mean(frac_drift),
    .groups = "drop"
  ) %>%
  mutate(
    # Weight by BOTH size AND quality (worse rate)
    sum_worse = sum(mean_worse), # add this otherwise you are ignoring the fact that ANY worse terminal node has small amount of worse in it
    weight = n_orig * mean_worse,
    proportion = weight / sum(weight),
    n_to_sample = round((1/sum_worse*n_per_worse)*proportion)
  )

print("\nWORSE tree nodes with quality weights:")
print(node_stats_worse %>% 
        select(node_worse, n_orig, mean_worse, weight, n_to_sample) %>%
        arrange(desc(mean_worse)))

# Calculate the RATE of drift outcomes in each node of the drift tree
node_stats_drift = df_with_nodes %>%
  ...
  summarise(
    ...
    mean_better = mean(frac_better),
    mean_worse = mean(frac_worse),
    mean_drift = mean(frac_drift),      # THIS IS THE KEY!
    .groups = "drop"
  ) %>%
  mutate(
    # Weight by BOTH size AND quality (drift rate)
    weight = n_orig * mean_drift,
    proportion = weight / sum(weight),
    n_to_sample = round(n_per_drift * proportion)
  )

print("\nDRIFT tree nodes with quality weights:")
print(node_stats_drift %>% 
        select(node_drift, n_orig, mean_drift, weight, n_to_sample) %>%
        arrange(desc(mean_drift)))
#-------------------------------------------------------------------------------
