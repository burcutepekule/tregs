...
df_clustering = df_clustering %>%
  mutate(
    frac_select = frac_better #or -, if truly looking for biological selection?
  )
...
png('tree_better.png', width=2800,height=1800,units = 'px')
plot(ctree_model, 
...
ctree_model_better=ctree_model
saveRDS(ctree_model_better,'ctree_model_better.rds')
