...
Uses pre-sampled parameters from balanced_lhs_parameters_123.csv,
generates scenarios, runs 100 replicates per parameter-scenario combination
using multiprocessing.

Usage:
    python mass_simulation_LHS_presampled.py --csv_file balanced_lhs_parameters_123.csv --n_cores 10 --output_dir results/
"""
...
def update_params_with_sampled_and_scenario(base_params, sampled_row, scenario_row):
    ...
    params["allow_tregs_to_do_their_job"] = int(
        scenario_row["allow_tregs_to_do_their_job"]
    )
    params["allow_tregs_to_suppress_cognate"] = bool(
        scenario_row["allow_tregs_to_suppress_cognate"]
    )
...
def run_single_simulation(args):
    ...
    try:
        ...
        print(
            f"ERROR in param_set={param_set_id}, scenario={scenario_id}, replicate={replicate_id}: {e}"
        )
        return None


def run_mass_simulations(
    csv_file,
    n_replicates=100,
    n_cores=10,
    output_dir="mass_sim_results_presampled_123",
    base_seed=42,
    ...
    for _, param_row in params_df.iterrows():
        ...
        param_set_file = (
            output_path / f"simulation_results_param_set_{param_set_id}.csv"
        )
        ...
        for _, scenario_row in scenarios_df.iterrows():
            ...
            # Update parameters
            params = update_params_with_sampled_and_scenario(
                base_params, param_row, scenario_row
            )
        ...
        with Pool(processes=n_cores) as pool:
            ...
            for i, result in enumerate(
                pool.imap_unordered(run_single_simulation, tasks), 1
            ):
        ...
        print(
            f"  Progress: {completed_param_sets}/{n_param_sets} sets ({100*completed_param_sets/n_param_sets:.1f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--csv_file",
        type=str,
        default="balanced_lhs_parameters_123.csv",
        help="CSV file with pre-sampled parameters (default: balanced_lhs_parameters_123.csv)",
    )
    parser.add_argument(
        "--n_replicates",
        type=int,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="mass_sim_results_presampled_123",
        help="Output directory (default: mass_sim_results_presampled_123)",
    )
    parser.add_argument(
