...
steady_state_idx = function(x, k = 20, tail_frac = 0.1,
                            tol_abs = 0.02 * (150 - 25),     # 2.5 units
                            tol_sd  = 0.5  * 0.02 * (150-25),# 1.25 units
                            tol_slope = 0.005 * (150 - 25))  # 0.125/step
{
  # Estimate the asymptote from the tail (e.g., median of the last 10%).
  # Declare steady state when, for K consecutive points, the rolling mean is 
  # within a small absolute band around the asymptote, and the rolling 
  # SD and slope are tiny.
  # 
  # Because epithelial score lives in [25,150] 
  # (range =125), good starting tolerances are:
  # absolute band ≈ 2% of range → 2.5 units
  # slope threshold ≈ 0.1% of range per step → 0.125 units/step
  ...
  cand = which( abs(m - x_asym) <= tol_abs &
                  sd <= tol_sd &
                  abs(sl) <= tol_slope )
...
scenarios      = read_csv('/Users/burcutepekule/Desktop/tregs/mass_sim_results_full_range/scenarios.csv', show_col_types = FALSE)

# Load all files
params                     = read_csv('/Users/burcutepekule/Desktop/tregs/mass_sim_results_full_range/sampled_parameters.csv', show_col_types = FALSE)
results_merged             = c()
sterile_comparison_keep    = c()
pathogenic_comparison_keep = c()


path      = "/Users/burcutepekule/Desktop/tregs/mass_sim_results_full_range/"
...
if(file.exists('/Users/burcutepekule/Desktop/tregs/all_comparison_results_max_id_full_range.rds')){
  start_id = 1+readRDS('/Users/burcutepekule/Desktop/tregs/all_comparison_results_max_id_full_range.rds')
}else{
  start_id = 0
}
...
for (i in start_id:max_index){
  ...
  results         = read_csv(paste0('/Users/burcutepekule/Desktop/tregs/mass_sim_results_full_range/simulation_results_param_set_',i,'.csv'), show_col_types = FALSE)
  ...
  for (rep in 0:9){
    ...
    if(!any(is.na(time_ss_vec))){
      ...
      median_diff_01 = median(scores_01_1) - median(scores_01_0)
      ...
      median_diff_12 = median(scores_12_2) - median(scores_12_1)
      ...
      median_diff_34 = median(scores_34_4) - median(scores_34_3)
      ...
      median_diff_45 = median(scores_45_5) - median(scores_45_4)
      ...
      comparison_results = data.frame(
        ...
        median_diff = c(median_diff_01, median_diff_12, median_diff_34, median_diff_45),
...
saveRDS(max(all_comparison_results$param_set_id), '/Users/burcutepekule/Desktop/tregs/all_comparison_results_max_id_full_range.rds') 

#---- read previous file
if(!file.exists('/Users/burcutepekule/Desktop/tregs/all_comparison_results_0_full_range.rds')){
  saveRDS(all_comparison_results, '/Users/burcutepekule/Desktop/tregs/all_comparison_results_0_full_range.rds')
}else{
  all_comparison_results_old = readRDS('/Users/burcutepekule/Desktop/tregs/all_comparison_results_0_full_range.rds')
