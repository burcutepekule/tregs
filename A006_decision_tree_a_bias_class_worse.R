...
ctree_model = ctree(dominant_outcome ~ .,
                    ...
                    control = ctree_control(
                      ...
                      maxdepth = 6,               # Max tree depth (0 = unlimited)
...
objective_function = function(random_integers, node_summary_counts) {
  ...
  # diff_rat = rep(1/3, 3) - c(better_rat, worse_rat, drift_rat)
  diff_rat = c(0,1,0) - c(better_rat, worse_rat, drift_rat)
...
# Shuffle rows
set.seed(123)
balanced_lhs_dataset = balanced_lhs_dataset %>% slice_sample(n = nrow(.))
...
# Export
write.csv(balanced_lhs_dataset, "balanced_lhs_parameters_worse.csv")
...
ggsave("parameter_distributions_comparison_worse.png", 
       density_plot, 
       width = 16, 
       height = 12, 
       dpi = 300)
