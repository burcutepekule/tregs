...
for (seed_in in 0:9){
  ...
  if (use_synchronized_rng) {
    random_stream_file <- paste0("random_numbers_seed_",seed_in,".csv")
    ...
    if (!file.exists(random_stream_file)) {
      ...
      rng_env <- new.env()
      rng_env$stream <- scan(random_stream_file, quiet = TRUE, skip = 1)
      rng_env$index <- 1
  ...
  if (param_set_id_use > nrow(params_df)) {
    stop(sprintf("Parameter row %d exceeds available rows (%d)", param_set_id_use, nrow(params_df)))
  }
    ...
    if (length(active_tregs) > 0) {
      for (i in active_tregs) { # add *allow_tregs to the end to truly check same randomness
        SAMPs[treg_y[i], treg_x[i]] = SAMPs[treg_y[i], treg_x[i]] +
          treg_activity_SAMPs_binary[i]*add_SAMPs
      }
    }
  ...
  longitudinal_df = longitudinal_df %>%
    select(t, sterile, allow_tregs_to_do_their_job, allow_tregs_to_suppress_cognate,
           randomize_tregs, param_set_id, everything())
  
  saveRDS(longitudinal_df, paste0('./longitudinal_df_',seed_in,'_tregs_',allow_tregs,'.rds'))
