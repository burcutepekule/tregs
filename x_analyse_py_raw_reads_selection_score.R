...
df_raw = df_raw %>% dplyr::mutate(outcome = ifelse(better==0 & worse==0, 'drift',
                                                   ifelse(better == 0 & worse == 1, 'worse',
                                                          ifelse( better == 1 & worse == 0, 'better', 'conflicting'))))


df_summary = df_raw %>%
  dplyr::group_by(param_set_id, comparison, injury_type, outcome) %>%
...
df_summary = df_summary %>% dplyr::mutate(selection_score = (0*drift+-1*worse+1*better)/(drift+worse+better))
saveRDS(df_summary, '/Users/burcutepekule/Desktop/tregs/df_summary_selection_score.rds')
