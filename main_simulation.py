...
def initialize_parameters():
    ...
    # Treg parameters
    params["treg_vicinity_effect"] = (
        1  # if 0, that means has to be at the very same pixel
    )
...
def run_simulation(params, random_seed=42):
    ...
    activity_engulf_M1_step = (
        params["activity_engulf_max"] - params["activity_engulf_M1_baseline"]
    ) / params["cc_phagocyte"]
    activity_engulf_M2_step = (
        params["activity_engulf_max"] - params["activity_engulf_M2_baseline"]
    ) / params["cc_phagocyte"]
    activity_ROS_M1_step = (
        params["activity_ROS_max"] - params["activity_ROS_M1_baseline"]
    ) / params["cc_phagocyte"]
    ...
    phagocyte_activity_engulf = np.full(
        n_phagocytes, params["activity_engulf_M0_baseline"], dtype=np.float32
    )
    phagocyte_activity_ROS = np.full(
        n_phagocytes, params["activity_ROS_M0_baseline"], dtype=np.float32
    )
    phagocyte_bacteria_registry = np.zeros(
        (n_phagocytes, params["cc_phagocyte"]), dtype=np.int32
    )
    ...
    pathogen_x = (
        np.random.choice(injury_site, n_pathogens_lp, replace=True)
        if n_pathogens_lp > 0
        else np.array([])
    )
    pathogen_y = np.zeros(n_pathogens_lp, dtype=np.int32)
    pathogen_coords = (
        np.column_stack([pathogen_x, pathogen_y])
        if n_pathogens_lp > 0
    ...
    commensal_y = np.random.randint(
        0, grid_size, params["n_commensals_lp"]
    )  # Random y position
    ...
    macrophages_longitudinal = np.zeros(
        (t_max, 1 + 2 * (params["cc_phagocyte"] + 1)), dtype=np.int32
    )
    ...
    for t in range(t_max):
        ...
        if np.any(active_tregs_mask):
            active_treg_positions = np.column_stack(
                [treg_y[active_tregs_mask], treg_x[active_tregs_mask]]
            )
        ...
        if len(pathogen_coords) > 0:
            ...
            dy = np.where(
                ...
                np.random.choice([-1, 0, 1], size=len(pathogen_coords), replace=True),
            )  # Random movement
            dx = iszero_coordinates(dy)
            pathogen_coords[:, 0] = np.clip(
                pathogen_coords[:, 0] + dx, 0, grid_size - 1
            )
            pathogen_coords[:, 1] = np.clip(
                pathogen_coords[:, 1] + dy, 0, grid_size - 1
            )

        if len(commensal_coords) > 0:
            # When at y=0 (touching epithelium), can only move uLP (dy=1), not lumen
            dy = np.where(
                commensal_coords[:, 1] == 0,
                np.ones(len(commensal_coords), dtype=np.int32),  # Must move up
                np.random.choice([-1, 0, 1], size=len(commensal_coords), replace=True),
            )  # Random movement
            dx = iszero_coordinates(dy)
            commensal_coords[:, 0] = np.clip(
                commensal_coords[:, 0] + dx, 0, grid_size - 1
            )
            commensal_coords[:, 1] = np.clip(
                commensal_coords[:, 1] + dy, 0, grid_size - 1
            )
        ...
        if np.any(bacteria_mask):
            bacteria_damage = logistic_scaled_0_to_5_quantized(
                bacteria_at_epithelium[bacteria_mask], params["k_in"], params["x0_in"]
            )
            DAMPs[0, bacteria_mask] += bacteria_damage * params["add_DAMPs"]

        # Diffuse signals BEFORE decay (like R code)
        DAMPs = diffuse_matrix(
            DAMPs, params["diffusion_speed_DAMPs"], params["max_cell_value_DAMPs"]
        )
        SAMPs = diffuse_matrix(
            SAMPs, params["diffusion_speed_SAMPs"], params["max_cell_value_SAMPs"]
        )
        ROS = diffuse_matrix(
            ROS, params["diffusion_speed_ROS"], params["max_cell_value_ROS"]
        )
        ...
        if len(injured_sites) > 0:
            ...
            n_pathogens_new = int(
                np.round(
                    mean_injury
                    * params["rate_leak_pathogen_injury"]
                    * len(injured_sites)
                )
            )
            n_commensals_injury = int(
                np.round(
                    mean_injury
                    * params["rate_leak_commensal_injury"]
                    * len(injured_sites)
                )
            )
            ...
            if n_pathogens_new > 0:
                ...
                probs = (
                    epithelium_injury / epithelium_injury.sum()
                    if epithelium_injury.sum() > 0
                    else None
                )
                new_x = np.random.choice(
                    grid_size, n_pathogens_new, replace=True, p=probs
                )
                new_pathogens = np.column_stack(
                    [new_x, np.zeros(n_pathogens_new, dtype=np.int32)]
                )
                pathogen_coords = (
                    np.vstack([pathogen_coords, new_pathogens])
                    if len(pathogen_coords) > 0
                    else new_pathogens
                )

        # Commensals leak from both injury and baseline
        n_commensals_baseline = int(
            np.round(params["rate_leak_commensal_baseline"] * grid_size)
        )
        ...
        if total_new_commensals > 0:
            ...
            if n_commensals_injury > 0:
                ...
                injury_x = np.random.choice(
                    grid_size, n_commensals_injury, replace=True, p=probs
                )
            ...
            if len(new_commensals_x) > 0:
                ...
                commensal_coords = (
                    np.vstack([commensal_coords, new_commensals])
                    if len(commensal_coords) > 0
                    else new_commensals
                )
        ...
        for i in M0_indices:
            ...
            avg_DAMP = get_8n_avg_signal_fast(
                px, py, params["act_radius_DAMPs"], DAMPs, grid_size
            )
            avg_SAMP = get_8n_avg_signal_fast(
                px, py, params["act_radius_SAMPs"], SAMPs, grid_size
            )
            ...
            if avg_DAMP >= params["activation_threshold_DAMPs"] and avg_DAMP > avg_SAMP:
                ...
                phagocyte_activity_ROS[i] = (
                    params["activity_ROS_M1_baseline"]
                    + activity_ROS_M1_step * bacteria_count
                )
                phagocyte_activity_engulf[i] = (
                    params["activity_engulf_M1_baseline"]
                    + activity_engulf_M1_step * bacteria_count
                )
            elif (
                avg_SAMP >= params["activation_threshold_SAMPs"] and avg_SAMP > avg_DAMP
            ):
                # Activate to M2
                ...
                phagocyte_activity_engulf[i] = (
                    params["activity_engulf_M2_baseline"]
        ...
        for i in active_indices:
            ...
            if phagocyte_active_age[i] >= params["active_age_limit"]:
                ...
                if (
                    avg_DAMP >= params["activation_threshold_DAMPs"]
                    and avg_DAMP > avg_SAMP
                ):
                    # Transition/stay M1
                    phagocyte_phenotype[i] = 1
                    phagocyte_active_age[i] = 1
                    ...
                    avg_SAMP >= params["activation_threshold_SAMPs"]
                    and avg_SAMP > avg_DAMP
                ):
                    # Transition/stay M2
                    phagocyte_phenotype[i] = 2
                    phagocyte_active_age[i] = 1
                    ...
                    avg_SAMP < params["activation_threshold_SAMPs"]
                    and avg_DAMP < params["activation_threshold_DAMPs"]
                ):
                    # Return to M0
        ...
        for i in range(n_phagocytes):
            ...
            if len(pathogen_coords) > 0:
                pathogen_indices = np.where(
                    (pathogen_coords[:, 0] == px) & (pathogen_coords[:, 1] == py)
                )[0]
                if len(pathogen_indices) > 0:
                    engulf_success = (
                        np.random.rand(len(pathogen_indices))
                        < phagocyte_activity_engulf[i]
                    )
                    ...
                    if len(indices_to_engulf) > 0:
                        ...
                        pathogens_killed_by_Mac[phagocyte_phenotype[i]] += len(
                            indices_to_engulf
                        )

            # Check for commensals at this location
            if len(commensal_coords) > 0:
                commensal_indices = np.where(
                    (commensal_coords[:, 0] == px) & (commensal_coords[:, 1] == py)
                )[0]
                if len(commensal_indices) > 0:
                    engulf_success = (
                        np.random.rand(len(commensal_indices))
                        < phagocyte_activity_engulf[i]
                    )
                    ...
                    if len(indices_to_engulf) > 0:
                        ...
                        commensal_coords = np.delete(
                            commensal_coords, indices_to_engulf, axis=0
                        )
                        ...
                        # Update kill count by phagocyte phenotype
                        commensals_killed_by_Mac[phagocyte_phenotype[i]] += len(
                            indices_to_engulf
                        )
                ...
                if phagocyte_phenotype[i] == 1:  # M1
                    ...
                    if len(nearby_tregs) > 0:
                        ...
                        if (num_pat_antigens + num_com_antigens) > 0:
                            rat_com_pat_real = num_com_antigens / (
                                num_com_antigens + num_pat_antigens
                            )

                            # Use GLOBAL treg_discrimination_efficiency for ALL Tregs
                            # This is the same for every Treg, not individualized
                            alpha = (
                                1 - params["treg_discrimination_efficiency"]
                            ) + params["treg_discrimination_efficiency"] * (
                                rat_com_pat_real * precision
                            )
                            beta = (
                                1 - params["treg_discrimination_efficiency"]
                            ) + params["treg_discrimination_efficiency"] * (
                                (1 - rat_com_pat_real) * precision
                            )
                            ...
                            if rat_com_pat > params["rat_com_pat_threshold"]:
                                ...
                                if params["allow_tregs_to_suppress_cognate"]:
                                    ...
                                    phagocyte_phenotype[i] = 2
                                    phagocyte_active_age[i] = 1
                                    bacteria_count = np.sum(
                                        phagocyte_bacteria_registry[i, :] > 0
                                    )
                                    phagocyte_activity_ROS[i] = params[
        ...
        if len(pathogen_coords) > 0:
            ...
            if len(epithelium_pathogens) > 0:
                unique, counts = np.unique(
                    epithelium_pathogens[:, 0], return_counts=True
                )
        ...
        if len(commensal_coords) > 0:
            ...
            if len(epithelium_commensals) > 0:
                unique, counts = np.unique(
                    epithelium_commensals[:, 0], return_counts=True
                )
        ...
        for i in range(grid_size):
            ...
            epithelium_injury[i] += int(
                logistic_scaled_0_to_5_quantized(
                    count_pathogens, params["k_in"], params["x0_in"]
                )
            )
            ...
            if (
                epithelium_injury[i] > 0
                and np.random.random() < params["epith_recovery_chance"]
            ):
        ...
        if np.any(m1_mask):
            ...
            m1_counts = np.bincount(m1_ages, minlength=params["cc_phagocyte"] + 1)[
                : params["cc_phagocyte"] + 1
            ]
        else:
        ...
        if np.any(m2_mask):
            ...
            m2_counts = np.bincount(m2_ages, minlength=params["cc_phagocyte"] + 1)[
                : params["cc_phagocyte"] + 1
            ]
        else:
        ...
        # Treg counts
        tregs_longitudinal[t, :] = [
            np.sum(treg_phenotype == 0),
            np.sum(treg_phenotype == 1),
        ]

        # Death counts (cumulative for this implementation)
        microbes_cumdeath_longitudinal[t, :] = [
            commensals_killed_by_ROS,
            commensals_killed_by_Mac[0],
            commensals_killed_by_Mac[1],
            commensals_killed_by_Mac[
                2
            ],  # Commensals (M0, M1, M2 not tracked separately)
            pathogens_killed_by_ROS,
            pathogens_killed_by_Mac[0],
            pathogens_killed_by_Mac[1],
            pathogens_killed_by_Mac[2],  # Pathogens (M0, M1, M2 not tracked separately)
        ]
...
def plot_results(df):
    ...
    ax = axes[0, 1]
    ax.plot(
        df["t"], df["commensal"], label="Commensals", color="turquoise", linewidth=2
    )
    ...
    ax.plot(
        df["t"],
        df[m1_cols].sum(axis=1),
        label="M1 (total)",
        color="magenta",
        linewidth=2,
    )
    ax.plot(
        df["t"], df[m2_cols].sum(axis=1), label="M2 (total)", color="green", linewidth=2
    )
    ax.set_xlabel("Time")
    ax.set_ylabel("Count")
    ...
    plt.tight_layout()
    plt.savefig(
        "/Users/burcutepekule/Desktop/tregs/simulation_results_py.png",
        dpi=150,
        bbox_inches="tight",
    )
    # plt.show()
...
if __name__ == "__main__":
    ...
    # Save results
    results_df.to_csv(
        "/Users/burcutepekule/Desktop/tregs/simulation_results_py.csv", index=False
    )
